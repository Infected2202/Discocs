...
4. Issue считается закрытым только после проверки поведения и исчезновения ключа
   ...
   готовности.
5. Старую админку не менять без отдельного явного решения Сани: правило проекта
   разрешает UI-работу по умолчанию только в `ui/src/`.
...
3. Принять отдельное решение по 56 issues старой админки:
   - поддерживаем и исправляем отдельным этапом;
   - оставляем в анализе как осознанный долг;
   - исключаем `app/ui.html` из SonarQube, если `/admin` выводится из поддержки.
...
- `6550f731-6547-454d-8810-5073f0460bd3` — root в
  `deploy/ci/Dockerfile.ui-test`. Проверить threat model CI-контейнера; вероятный
  итог — `ACKNOWLEDGED`, если контейнер одноразовый и непривилегированный, либо
  добавить пользователя и проверить pnpm cache/build.
- `1c29092e-24a1-481a-b049-fa334e4a9c64` — `random.shuffle` в dashboard.
  ...
  после подтверждения тестом назначения вероятный итог — `SAFE`.
- `a24e89a3-82ef-4e93-ad11-fdf816782d96` — потенциальный clear-text URL,
  передаваемый worker через `DISCOCS_WORKER_SERVER`. Нужно решить, разрешён ли
  HTTP только внутри доверенной сети, или production-конфигурация обязана
  требовать HTTPS.
- `04a845ed-ddd6-4382-b08f-1ae71aaf27c8` — MD5 в Subsonic-аутентификации бота.
  Алгоритм задан протоколом Navidrome/Subsonic; проверить транспорт и отсутствие
  логирования секрета, затем, вероятно, `ACKNOWLEDGED`.
...
- `a7297125-93b4-4802-9b0f-b5deac5d2e7a` — сравнение нормы float с нулём:
  определить требуемую epsilon-семантику, не подставлять порог наугад.
...
### Этап 6. Старая админка `/admin`

Запускается только после явного решения Сани на этапе 0. В ней 56 issues, включая
1 vulnerability, 5 bugs, accessibility, nested ternaries/templates и две
complexity-функции.

Если админка поддерживается:

1. SRI и runtime bugs;
...
Если админка не поддерживается, предпочтительно удалить её из runtime и анализа
отдельным осознанным изменением, а не просто скрыть 56 issues через exclusion.
...
9. Bot complexity.
10. Отдельная порция для `/admin`, только если разрешена.
...
### Старая админка `app/ui.html` — только после явного разрешения
...
- `6550f731-6547-454d-8810-5073f0460bd3` — `deploy/ci/Dockerfile.ui-test`, `docker:S6471`.
- `1c29092e-24a1-481a-b049-fa334e4a9c64` — `app/services/dashboard.py`, `python:S2245`.
- `a24e89a3-82ef-4e93-ad11-fdf816782d96` — `deploy/worker/Dockerfile`, `docker:S5332`.
- `04a845ed-ddd6-4382-b08f-1ae71aaf27c8` — `discocs_bot/bot/services/navidrome.py`, `python:S4790`.
