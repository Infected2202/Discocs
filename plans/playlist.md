...
7. **Фаза 7 — reorder** (§5.6, опционально по итогам приёмки) +
   документация `docs/`. ✅ (dnd-kit: `DragOverlay` — строка липнет к
   курсору, соседи расступаются live через `arrayMove`; pointer/touch/
   keyboard-сенсоры, совместимо с виртуализацией VirtualTrackList.
   Чекбоксы выбора вынесены в отдельную колонку справа, кнопка play
   слева больше не подменяется.)
