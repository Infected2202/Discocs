...
MASTER without a beat-grid/source-kind precondition; while any deck plays,
CLOCK cannot be selected. With both decks playing, MASTER can move between
them. A SYNC follower waits for its pending Signalsmith preparation and reports
a concrete failure if that preparation cannot complete.
