...
function fixture() {
  ...
  new Float32Array(payload, 20, 1)[0] = 0.5
  new Float32Array(payload, 24, 1)[0] = 120
  const descriptor = (offset: number, dtype: "int16" | "uint16") => ({ offset, length: 2, dtype, scale: 1, unit: "normalized" })
  return { payload, manifest: {
    schema_version: 1, pack_name: "timeline_foundation", extractor: "timeline_foundation_v2", duration_seconds: 2,
    ...
    rhythm: { bpm: 120, confidence: 0.8, coverage_seconds: 2, arrays: {
      beats: { offset: 20, length: 1, dtype: "float32", scale: 1, unit: "seconds" },
      local_tempo: { offset: 24, length: 1, dtype: "float32", scale: 1, unit: "bpm" },
    } },
    payload: { byte_length: 28, sha256: "00", endianness: "little", descriptor_alignment: 4 },
  } }
}
