...
function SectionLabel({ label, count, muted }: { label: string; count?: number; muted?: boolean }) {
  return (
    <div className={cn("px-4 py-1.5 flex items-center gap-2", muted ? "bg-muted/20" : "bg-muted/30")}>
      <span className={cn("text-xs font-semibold uppercase tracking-wider", muted ? "text-muted-foreground" : "text-foreground/70")}>
        {label}
      </span>
      {count != null && (
...
function QueueTrackRow({ track, trackId, isCurrent, dimmed, time, onClick }: QueueTrackRowProps) {
