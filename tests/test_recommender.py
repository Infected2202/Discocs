...
def test_artist_credit_keys_splits_collaborators():
    ...
    # Bare `&`/`;`/`+` always split, with or without surrounding whitespace —
...
def test_artist_credit_split_regex_handles_whitespace_flood_without_hanging():
    # Regression for S5852: the old `\s+...\s+|\s*[;&+]\s*` pattern wrapped
    # each alternative in an unbounded quantifier, so re.split had to rescan
    # the remaining whitespace run from every position once no delimiter
    # followed — quadratic in the run length. _metadata_key() collapses
