...
The five-container worker Compose profile keeps at most two CPU analyses and
two claimed tasks in flight in each container. A larger four-task buffer did
not improve steady throughput and increased download/submit tail latency, so
downloads and result submissions remain unbatched (`1`). The worker reuses a
bounded HTTP connection pool for claim, download, state and submit requests;
this removes per-track TCP setup without changing task payloads or audio
analysis.
