...
(`network_mode: service:awg`) идёт через amneziawg-туннель. Секрет здесь только
конфиг туннеля (`awg0.conf` с ключами) — он кладётся руками в
`${DISCOCS_STATE_DIR}/discocs_awg/` и не коммитится. А вот entrypoint
`start-awg.sh` живёт в git (`deploy/prod/start-awg.sh`): стадия `Deploy`
раскладывает его на хост рядом с конфигом до `up`, потому что bind-mount
подхватывает содержимое файла в момент создания контейнера. Правки делать в
репозитории, не на сервере. `backend` по-прежнему резолвится по имени, т.к.
`awg` сидит в той же compose-сети.

Туннель и бот следят за собой сами — healthcheck по возрасту handshake, сторож
доступности Telegram в боте, ступенчатое самолечение сайдкара. Подробности и
пороги: `docs/operations.md`, раздел «Бот: туннель, наблюдаемость,
самовосстановление».
...
Отчётная часть строится из **одного** разбора образа: скан пишет JSON
(`trivy image --format json -o /report.json`), а обе читаемые формы делает
`trivy convert` над готовым файлом — он не перечитывает слои и не обращается
к БД:
...
| блокирующий гейт | `trivy image --ignore-unfixed --severity HIGH,CRITICAL --exit-code 1` | полный скан |

Гейт остался отдельным сканом не по недосмотру: у `trivy convert` из фильтров
есть только `--severity`, `--exit-code` и `--ignore-policy`, а флага
`--ignore-unfixed` нет — билд #247 упал на `unknown flag: --ignore-unfixed`.
Отфильтровать unfixed можно Rego-политикой через `--ignore-policy`, но ошибка
в такой политике даёт **молча пропускающий** гейт, а это худший вид отказа для
проверки безопасности. Родной флаг стоит ~20с и не умеет ошибаться тихо.
...
завести релизный keystore отдельным Jenkins-credential'ом (**Secret file**) и
переключить сборку на `assembleRelease` с подписью через него.
